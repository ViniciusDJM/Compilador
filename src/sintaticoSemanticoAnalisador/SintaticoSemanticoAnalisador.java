...
public class SintaticoSemanticoAnalisador {
	...
	//lista para verificação semantica da equação partir de valores fornecidos
	List<Token> lookExpressionData = new ArrayList<Token>();
	//lista para verificação semantica da equação partir das variaveis
	...
	//metodo que realiza a comparação do que recebeu com o que era esperado
	//obs: quando quiser comparar numero passa o waited como null que ai ele faz a comparação pela TAG
	...
	//metodo para achar o token de sincromização
	...
	//metodo para verificar se todos os valoressão do mesmo tipo
	public boolean lookExpression(int type, boolean seeParamFunction) {
		...
		if(seeParamFunction) {
			...
			//verifica se foram passados valores do tipo daatribuição
			...
			//verifica se as variaveissão do mesmo tipo que daatribuição
		...
		if(!isEqualType) semanticErrors.add("Erro semântico: este valor não corresponde ao tipo de dado, na linha:" + lookahead.line);
		if(!isInitialiazed) semanticErrors.add("Erro semântico: foi usado variaveis que não foram inicializadas, na linha:" + lookahead.line);
		if(!isEqualTypeVariable) semanticErrors.add("Erro semântico: foi usado variaveis com o tipo de dado errado, na linha:" + lookahead.line);
	...
	//metodo para analisar a função start 
	...
	public void analyseConst(){
		...
		//função que realiza o procedimento de verificação de instancias de atributos
	...
	//metodo para analisar a função read
	...
	//metodo para analisar a função print
	public void analysePrint(String scope) {
			...
			while(true) {
				if(lookahead.tag == Tag.CAD) {
					...
					error("Esperava encontrar uma string ou umaexpressão na linha " + lookahead.line, symbSyncronization);
				}
	...
	public void assign(String scope) {
		...
		if(getLexeme(lookahead).equals("global") || getLexeme(lookahead).equals("local")) {
			...
			if(getLexeme(lookahead).equals(".")) {
							...
							while(count < 2) {
								if(getLexeme(lookahead).equals("[")) {
									...
									if(lookahead.tag == Tag.NRO || lookahead.tag == Tag.IDE) {
										...
										if(!isOnlyType)semanticErrors.add("Erro semântico: Na declaração de vetores os indices são do tipo int erro encontrado em:" + lookahead.line);
									}
									else {
										List<Token> symbSyncronization = Arrays.asList(
												new Word (",", Tag.DEL, -1),
				...
				if(symbolTable.containsKey(scope+ "_"+ assignTo)) {
					...
					semanticErrors.add("Erro semântico: essa variavel não existe, erro encontrado na linha:" + lookahead.line );
				}
			}
		}
		if(getLexeme(lookahead).equals("=") && lookahead.tag == Tag.REL) {
			...
			if(variableAssign == null) {
				...
				if(!isOnlyType) semanticErrors.add("Erro semântico: expressão de atribuição de maneira errada(variavel não inicializada ou tipo de dado errado) erro encontrado em:" + lookahead.line);
				if(isStruct) {
		...
		while(true){
			if(lookahead.tag == Tag.IDE){
				if(symbol.getAttributes().containsKey(getLexeme(lookahead)))
					semanticErrors.add("Erro Semântico (struct): Já existe essa variável, erro encontrado em:" + lookahead.line + getLexeme(lookahead));
				else if(existType) {
					symbol.add(getLexeme(lookahead), type);
				}
				else
					semanticErrors.add("Erro Semântico (struct): não existe esse tipo de variavel erro encontrado em:" + lookahead.line +  type);
				match(lookahead, null, Tag.IDE);
			}else {
				List<Token> symbSyncronization = Arrays.asList(
						new Word(" ",Tag.IDE,-1),
		...
		while(true) {
			if(lookahead.tag == Tag.CAD) {
				if(typeDataTableSymbol.get(type) != Tag.CAD) 
					semanticErrors.add("Erro semântico: alocagem de informação com o tipo de dado errado erro encontrado em:" + lookahead.line );
				match(lookahead, null, Tag.CAD);
			}
			else {
				if(typeDataTableSymbol.get(type) == Tag.CAD) {
					...
					if(!isOnlyType) semanticErrors.add("Erro semântico: alocagem de informação com o tipo de dado errado erro encontrado em:" + lookahead.line);
				}
			}
			if(getLexeme(lookahead).equals(",")) {
				match(lookahead,",",Tag.DEL);
			...
			if(getLexeme(lookahead).equals("function") && lookahead.tag == Tag.PRE) {
				...
				if(getLexeme(lookahead).equals(")") && lookahead.tag == Tag.DEL) {
					...
					if(!alreadyExist) {
						...
						if(!function.getParameters().equals(aux.getParameters())) {
							...
							semanticErrors.add("Erro semântico: não é permitido ter uma função com o mesmo nome e parâetros de uma outra função que já existe erro encontrado em:" + lookahead.line);
						}
					}
				...
				if(lookahead.tag == Tag.IDE) {
					...
					if(!functionTableSymbol.contains(new TabelaFuncao("", name, "", true))) {
						...
						semanticErrors.add("Já existe procedure com esse nome, erro encontrado na linha:" + lookahead.line);
					match(lookahead,null,Tag.IDE);
				}else {
					List<Token> symbSyncronization = Arrays.asList(
							new Word("(",Tag.DEL,-1),
				...
				if(!isProcedure) {
					...
					if(function.getType() != "string") {
						...
						if(function != null && !function.isProcedure()) {
							...
							if(!isOnlyOneType) semanticErrors.add("Não retorna o tipo de dado definido pela função: " + function.getName()+ ", erro encontrado em:" + lookahead.line );
						}else {
							semanticErrors.add("Erro semântico: O tipo do retorno está incorreto erro encontrado em:" + lookahead.line);
						}
					}else if(function.getType() == "string" && lookahead.tag != Tag.CAD) {
						expression(nameFunction,false);
						lookExpression(typeDataTableSymbol.get(function.getType()),false);
						semanticErrors.add("Não retorna o tipo de dado definido pela função, erro encontrado em:" + lookahead.line);
	...
	public void functionCall(String calledBy, String nameFunction, boolean isProcedure){
		...
		TabelaFuncao function; //referencia para a função
		Object typesOfParams[] = null; //referencia para os parametros da função
		nameFunction = isProcedure ? "proc_"+ nameFunction : "func_" + nameFunction;
		calledBy = isProcedure ? "proc_"+ calledBy : "func_" + calledBy;
		//verfica se existe um registro de uma função que contenha o nome passado como parametro na tabela de simbolos
		if(functionTableSymbol.containsKey(nameFunction)) {
			...
			function = null;
			semanticErrors.add("Não existe essa função " + nameFunction + "Erro encontrado na linha:" + lookahead.line);
		}
		if(getLexeme(lookahead).equals("(") && lookahead.tag == Tag.DEL) {
			match(lookahead,"(",Tag.DEL);
			//verifica se a função tem ou não parametros
			if(getLexeme(lookahead).equals(")") && lookahead.tag == Tag.DEL) {
				if(function != null) {
				...
				if(function != null) {
					...
					while(true){
						if(typeDataTableSymbol.get(typesOfParams[indexParam].toString()) == Tag.CAD) {
							if(lookahead.tag != Tag.CAD) {
								semanticErrors.add("Erro Semântico: parâmetros com tipos diferentes " + nameFunction + "erro encontrado em:" + lookahead.line);
							...
							if(lookahead.tag == Tag.CAD) {
								...
								if(!isOnlyOneType)semanticErrors.add("Erro Semântico: parâmetros com tipos diferentes (" + nameFunction + "erro encontrado em:" + lookahead.line); 
							}
						}
						if(getLexeme(lookahead).equals(",") && lookahead.tag == Tag.DEL) {
							match(lookahead,",",Tag.DEL);
					...
					if(indexParam != typesOfParams.length - 1)
						semanticErrors.add("Erro! Parâmetros ausentes " + nameFunction + " " + lookahead.line + function.getParameters().size());
					if(indexParam > typesOfParams.length + 1)
						semanticErrors.add("Erro! Parâmetros que não  " + nameFunction + " " + lookahead.line + function.getParameters().size());
		...
		if(lookahead.tag == Tag.IDE) {
			if(isGlobal) {
				if(!symbolTable.containsKey("global" + "_" + getLexeme(lookahead)))
					semanticErrors.add("Não existe essa variavel: " + getLexeme(lookahead) + ", erro encontrado na linha: " + lookahead.line);
				else {
					if(isToAssign)
	...
	public void prefixStruct(String nameVariable, String scope, boolean isAssign) {
		...
		if(symbolTable.containsKey(scope + "_" + nameVariable)) {
			...
			semanticErrors.add("Não existe essa variavel: " + nameVariable + ", erro encontrado na linha: " + lookahead.line);
		}
		match(lookahead,".",Tag.DEL);
		if(lookahead.tag == Tag.IDE) {
			...
			if(isAssign && structVariable != null) {
				if(structTableSymbol.get(structVariable.getTypeData()).getAttributes().containsKey(attribute)) {
					...
					semanticErrors.add("Erro semântico: não existe esse atributo nesta struct erro encontrado em:" + lookahead.line);
				}
				...
				if(symbolTable.containsKey(scope + "_" + nameVariable + "." + attribute)) {
					...
					if(structTableSymbol.get(structVariable.getTypeData()).getAttributes().containsKey(attribute)) {
						...
						semanticErrors.add("Erro semântico: não existe esse atributo na struct escolhida, erro encontrado na linha:" + lookahead.line);
					}
				}
			}
		}else {
		...
		if(getLexeme(lookahead).equals("global") || getLexeme(lookahead).equals("local")) {
			...
			if(getLexeme(lookahead).equals("(")) {
				...
				if(symbolTable.containsKey(scope + "_" + name)) {
					...
					semanticErrors.add("Variavel não existe" + name + " erro encontrado na linha" + lookahead.line);
			...
			error("Esperava encontrar um identificador,numero,cadeia de caracteres,true,false,global., local. ou - como valor dentro de uma expressão na linha " + lookahead.line, symbSyncronization);
		}
	}
	...
	public void attributeValue(String type, String scope) {
		...
		while(true){
			...
			if(getLexeme(lookahead).equals("=")) {
				...
				error("Esperava encontrar o simbolo de = no processo de atribuiação, na linha " + lookahead.line, symbSyncronization);
			}
			if(lookahead.tag == Tag.NRO || lookahead.tag == Tag.IDE || lookahead.tag == Tag.PRE || getLexeme(lookahead).equals("-")) {
				if(existType) {
					...
					boolean isOnlyType = lookExpression(typeDataTableSymbol.get(type),false);
					if(!isOnlyType)
						semanticErrors.add("Erro Semântico (const): atribuição incorreta" + lookahead.line);
				}else {
					expression(scope,false);
					lookExpressionData.clear();
					lookExpressionVariable.clear();
				...
				List<Token> symbSyncronization = Arrays.asList(
						...
						new Word(";",Tag.DEL,-1),
						new Word(",",Tag.DEL,-1));
				error("Esperava encontrar um numero,identificador,true,false ou uma cadeia de caracteres no processo de atribuição, na linha " + lookahead.line, symbSyncronization);
			}
	...
	public void attributeValueVar(String type, String scope) {
		...
		while(true) {
			...
			if(lookahead.tag == Tag.IDE){
				//serve para verificar se o var global e que não exista uma const com o mesmo nome
				...
				if(check || verify) {
					semanticErrors.add("Erro semântico (var): já exite variavel com esse nome, erro na linha:" + lookahead.line);
				}
				else if(existType) 
					symbol.setName(getLexeme(lookahead));
				else
					semanticErrors.add("Erro Semântico (const): não existe esse tipo de variavel " + type + "erro encontrado na linha: "+ lookahead.line);
				match(lookahead, null, Tag.IDE);
			}
			else {
				List<Token> symbSyncronization = Arrays.asList(
			...
			if(getLexeme(lookahead).equals("[")) {
				...
				while(count < 2) {
					if(getLexeme(lookahead).equals("[")) {
						...
						if(lookahead.tag == Tag.NRO || lookahead.tag == Tag.IDE) {
							expression(scope,false);
							boolean isOnlyType = lookExpression(Tag.NRO,false);
							if(!isOnlyType)semanticErrors.add("Erro semântico: Na declaração de vetores os indices são do tipo int, linha:" + lookahead.line);
						}
						else {
							List<Token> symbSyncronization = Arrays.asList(
									new Word (",", Tag.DEL, -1),
						...
						if(getLexeme(lookahead).equals("]")) {
							...
							error("Esperava encontrar um ] processo deatribuição de vetor ou matriz, na linha " + lookahead.line, symbSyncronization);
						}
					}
					else count = 2;
				}
				if(getLexeme(lookahead).equals("=")) {
					...
					if(getLexeme(lookahead).equals("{")) {
						match(lookahead, "{", Tag.DEL);
						if(getLexeme(lookahead).equals("{")) {
							if(dimension == 1) semanticErrors.add("Erro semântico: dimensão errada da alocação de memória na linha:" + lookahead.line);
							attributeMatrix(scope,type);
						}else {
							if(dimension == 2) semanticErrors.add("Erro semântico: dimensão errada da alocação de memória na linha:" + lookahead.line);
							attributeVector(scope,type);
						}
					}
				}
				...
				if(lookahead.tag == Tag.NRO || lookahead.tag == Tag.IDE || getLexeme(lookahead).equals("true") || getLexeme(lookahead).equals("false")) {
					if(existType) {
						...
						boolean isOnlyType = lookExpression(typeDataTableSymbol.get(type),false);
						if(!isOnlyType)
							semanticErrors.add("Erro Semântico (const): atribuição incorreta na linha:" + lookahead.line);
					}else {
						expression(scope,false);
						lookExpressionData.clear();
						lookExpressionVariable.clear();
